...
async function enviarFormulario(event) {
    try {
        ...
        const existingMoviesResponse = await axios.get(`http://localhost:3000/movies?title=${titleInput.value}`);
        const existingMovies = existingMoviesResponse.data;
        if (existingMovies.length > 0) {
            alert('La película ya está cargada.');
            return;
        }
        const genres = [];
        checkboxes.forEach(checkbox => {
